...
import NumberGrid from '@/components/pastime/NumberGrid';
import ScoreDisplay from '@/components/pastime/ScoreDisplay';
import { useNumberOrdering } from '@/hooks/useNumberOrdering';
import { useGame } from '@/hooks/useGame';

export default function NumberOrderingScreen() {
  const { numbers, nextExpected, isComplete, tapNumber, resetGame } =
    useNumberOrdering();
  const game = useGame();
  ...
  const handleTap = useCallback(
    (num: number) => {
      ...
      const correct = tapNumber(num);
      if (correct) {
        game.addScore(1);
      }
    },
    [game.isActive, tapNumber, game.addScore],
  );
      ...
      <View style={styles.content}>
        ...
        {!game.isActive && !game.isComplete ? (
          <View style={styles.startArea}>
            <Text style={styles.instruction}>1부터 16까지 순서대로 눌러주세요!</Text>
